...
from content_aggregator.clients.llm_utils import extract_json_from_text, strip_reasoning_thoughts
...
class LanguageDetector:
    ...
    def _parse_llm_response(self, response: str) -> LanguageDetectResult:
        """解析 LLM 返回的 JSON（使用健壮提取器）"""
        # 使用 extract_json_from_text 从 LLM 文本中提取 JSON
        # 会自动处理 <think>、```json 等包裹格式
        data = extract_json_from_text(response, expected_type=dict)
