...
严禁你在输出的结果开头出现相应我的任何回答,比如"好的,这是为您优化的短视频文案,严格遵循您的指令和要求:"这样类似的话,我需要你直接输出结果。""",
    ...
    def _build_prompt(self, content: Content, config: RewriteConfig) -> str:
        """构建完整的提示词"""
        # 清洗输入内容：移除推广信息、URL、社交账号等噪点
        # (从 Y2A-Auto ai_enhancer._pre_clean 移植)
        _clean_title = clean_input_content(content.title or "", max_blocks=1)
        _clean_body = clean_input_content(content.content or "", max_blocks=3)
...
"""
    ...
    def _parse_response(
        ...
        """解析 LLM 响应"""
        # 清理推理标签（兼容 <think> 和 ```think 两种格式）
        response = strip_reasoning_thoughts(response)
