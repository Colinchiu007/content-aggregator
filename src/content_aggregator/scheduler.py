...
class ContentScheduler:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self._task_store: TaskStore | None = None  # 断点续跑存储（由 init_checkpoint_store 设置）

    def init_checkpoint_store(self, db_path: str | None = None) -> "ContentScheduler":
        """初始化断点续跑存储。
        ...
        self._task_store = TaskStore(db_path)
        logger.info(f"[Scheduler] 断点续跑存储已初始化: {db_path}")
        return self
    ...
    def load_from_config(self, config: dict[str, Any]) -> None:
        ...
        for job in jobs:
            ...
            async def callback(cfg=config, jc=job, s=self):
                job_name = jc.get('name', 'unknown')
                ...
                try:
                    ...
                    async with ContentPipeline(cfg) as pipeline:
                        # 初始化断点续跑存储（复用调度器的 store）
                        if s._task_store:
                            pipeline._checkpoint_store = s._task_store

                        # 运行前检查：检测是否有同名的中断任务
                        if s._task_store:
                            interrupted = s._task_store.get_interrupted_tasks()
                            job_interrupted = [
                                t for t in interrupted
                                if t.scheduler_job == job_name or t.name == job_name
                            ]
                            for t in job_interrupted:
                                logger.info(f"[Scheduler] 检测到中断任务，跳过恢复（新周期运行）: {job_name}")
                                clear_task_cancel(t.id)

                        # 使用带断点续跑的包装器
                        result = await pipeline.process_all_sources_with_checkpoint(
                            task_name=job_name,
                        ...
                        logger.info(f"Job '{job_name}' completed: {summary}")
    ...
    async def start(self) -> None:
        ...
        # ⚡ 断点续跑: 启动时检测并恢复中断的任务
        if self._task_store:
            from content_aggregator.workflows.task_store import is_task_active
            interrupted = self._task_store.get_interrupted_tasks()
            if interrupted:
                logger.info(f"[Scheduler] 发现 {len(interrupted)} 个中断的任务")
                for task in interrupted:
                    # 不做自动恢复（scheduler restart = 新生命），仅记录
                    self._task_store.update_task_status(
                        task.id, TaskStatus.FAILED,
                        error="调度器重启，任务中断"
                    )
                    logger.info(f"[Scheduler] 标记中断任务为 failed: {task.name}")
    ...
    async def cancel_task(self, task_id: str) -> bool:
        ...
        返回：
            True 表示已请求取消
        """
        event = request_task_cancel(task_id)
        if self._task_store:
            self._task_store.update_task_status(task_id, TaskStatus.CANCELLED, error="用户取消")
        if event:
            # 等待最多 3 秒让任务响应
            event_set = event.wait(timeout=3.0)
            return event_set
        return True
    ...
    def get_checkpoint_status(self) -> dict[str, Any]:
        ...
        if not self._task_store:
            return {"enabled": False, "message": "未启用断点续跑"}

        interrupted = self._task_store.get_interrupted_tasks()
        active_summary = self._task_store.get_active_tasks_summary()

        return {
            "enabled": True,
            "db_path": self._task_store.db_path,
