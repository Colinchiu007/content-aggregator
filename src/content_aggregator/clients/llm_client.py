...
def _is_reasoning_content(content: str) -> bool:
    ...
    # 1. 以英文开头 → 可能是推理内容
    if c and c[0].isascii() and not c[0].isalpha():
        pass  # 继续检查其他条件
...
def _extract_final_from_reasoning(reasoning: str) -> str:
    """
    从推理内容中提取最终答案
    作为兜底方案——首先尝试 strip_reasoning_thoughts
    """
    if not reasoning:
        return ""

    # 优先：用 strip_reasoning_thoughts（更健壮的正则）
    cleaned = strip_reasoning_thoughts(reasoning)
...
class LLMClient:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # 解密 API Key（如果是 enc: 前缀），fallback 到 .env / 环境变量
        ...
        if not self.api_key:
            ...
            for env_key in [f"{model_id.upper().replace('-','_')}_API_KEY", "DEEPSEEK_API_KEY"]:
                val = get_secret(env_key)
                if val:
                    self.api_key = val
                    logger.info(f"[LLMClient] 从环境变量加载 API Key: {env_key}")
                    break
    ...
    async def _call_openai_compatible(self, prompt: str) -> dict:
        ...
        has_thinking_param = "sensenova" in self.model.lower() or "think" in self.model.lower()
        data = {
            "model": self.model,
        ...
        # 对推理模型（如 sensenova-6.7-flash-lite）尝试禁用思考模式
        # 如果 API 不支持 extra_body.thinking 参数，会回退（见下方 except）
        if has_thinking_param:
            data = build_thinking_disabled_body(data)
            logger.info(f"[_call_openai_compatible] 已添加 thinking=disabled 参数")
        ...
        # 重试逻辑
        last_error = None
        has_retried_without_thinking = False
        for attempt in range(self.retry):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # 先去 think 标签
                    cleaned_content = strip_reasoning_thoughts(raw_content)

                    # 决定最终使用的 content
                    content = cleaned_content

                    if not content or _is_reasoning_content(content):
                        # content 仍为推理内容 → 尝试从 reasoning 字段提取
                        extracted = _extract_final_from_reasoning(reasoning)
                        if extracted:
                            content = extracted
                        else:
                            # reasoning 也无法提取，使用清洗后的 raw_content
                            content = cleaned_content
                    ...
                    # 检查是否是 thinking 参数不被支持 → 降级重试
                    if (
                        has_thinking_param
                        and not has_retried_without_thinking
                        and any(
                            sig in error_text
                            for sig in [
                                "unknown parameter",
                        ...
                        logger.warning(
                            "[_call_openai_compatible] API 不支持 thinking 参数，降级重试"
                        )
                        # 移除 extra_body 重试
                        data.pop("extra_body", None)
                        has_retried_without_thinking = True
                        continue
