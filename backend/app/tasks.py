...
# ── ca_publish_to_platform: 异步执行平台发布 ──

def _publish_to_platform_factory(celery_app):
    """创建 ca_publish_to_platform Celery 任务（或占位函数）"""
    if celery_app is None:
        def placeholder_task(*args, **kwargs):
            logger.warning("ca_publish_to_platform: Celery 不可用，任务无法执行")
            return None
        placeholder_task.delay = lambda *args, **kwargs: None
        placeholder_task.apply_async = lambda *args, **kwargs: None
        return placeholder_task

    @celery_app.task(bind=True, max_retries=3, default_retry_delay=60, name="ca_publish_to_platform")
    def ca_publish_to_platform(self, article_id: str, platform: str):
        """异步执行平台发布 — 调用 orchestrator publish API"""
        import asyncio
        from app.services.publisher import _execute_platform_publish

        logger.info(f"[ca_publish_to_platform] 开始发布: article_id={article_id}, platform={platform}")
        try:
            result = asyncio.run(_execute_platform_publish(
                article_id=article_id,
                platform=platform,
            ))
            logger.info(f"[ca_publish_to_platform] 发布完成: {result}")
            return result
        except Exception as e:
            logger.error(f"[ca_publish_to_platform] 发布失败: {e}")
            countdown = 60 * (2 ** self.request.retries)
            raise self.retry(exc=e, countdown=countdown)

    return ca_publish_to_platform
...
ca_publish_to_platform = _publish_to_platform_factory(celery_app)
