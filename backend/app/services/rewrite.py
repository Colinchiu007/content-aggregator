...
Moved from platform-orchestrator/services/rewrite.py (P3-03).
Standalone — takes raw content string, no database dependency.
Uses PO_OPENAI_API_KEY / PO_OPENAI_BASE_URL / PO_OPENAI_MODEL env vars.
"""
...
STYLE_PROMPTS: dict[str, str] = {
    ...
    "认知锚点": (
        "你是一位擅长认知锚点提炼的内容架构师。"
        "请将以下文章改写成「认知锚点」结构化的风格，在改写过程中提炼并嵌入5种认知锚点：\n\n"
        "1. 【核心判断】—— 文章的核心观点或关键结论，用一句话提炼。格式：「核心判断：…」\n"
        "2. 【断点】—— 认知转折点，读者原有的认知被打破或刷新之处。格式：「断点：原来…」\n"
        "3. 【分流】—— 不同观点、路径或可能性的分岔点。格式：「分流：一方面…另一方面…」\n"
        "4. 【对比】—— 两个事物、时期或观点的对比，突出差异。格式：「对比：A vs B」\n"
        "5. 【变化】—— 趋势、状态或认知的变化轨迹。格式：「变化：从…到…」\n\n"
        "输出要求：\n"
...
async def rewrite_content(
    ...
    Args:
        ...
        api_key: Override LLM API key (defaults to PO_OPENAI_API_KEY).
        base_url: Override LLM base URL (defaults to PO_OPENAI_BASE_URL).
        model: Override LLM model name (defaults to PO_OPENAI_MODEL).
    ...
    key = api_key or os.environ.get("PO_OPENAI_API_KEY", "")
    url = base_url or os.environ.get("PO_OPENAI_BASE_URL", "https://api.openai.com/v1")
    mdl = model or os.environ.get("PO_OPENAI_MODEL", "gpt-4o-mini")
